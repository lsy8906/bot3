...
# 								color=0x00ff00
# 								)
						await self.bot.get_channel(channel).send("```" + fixed_bossData[i][0] + fixed_bossData[i][4] + "```", tts=False)
						try:
							if basicSetting[21] == "1":
...
# 											color=0xff0000
# 											)
										await self.bot.get_channel(channel).send('```다음 ' + bossData[i][0] + ' ' + bossTimeString[i] + '입니다.```', tts=False)
										try:
											if basicSetting[21] == "1":
